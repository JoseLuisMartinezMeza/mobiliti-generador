...
_CELL = r"\$?[A-Z]{1,3}\$?[1-9][0-9]*"
_SHEET = r"(?:'(?:[^']|'')+'|[^'!]+)!"
_RANGE_REFERENCE = re.compile(
    rf"^(?:{_SHEET})?{_CELL}(?::{_CELL})?$", re.IGNORECASE
)
...
def translate_calc_chain(
    ...
    *,
    ...
    existing_target_coordinates = {
        cell.attrib.get("r")
        for cell in cells
        if cell.attrib.get("i") == target_sheet_id
    }

    for cell in cells:
        source = cell.attrib.get("r")
        if cell.attrib.get("i") != target_sheet_id or source not in coordinate_map:
            output.append(cell)
            continue
