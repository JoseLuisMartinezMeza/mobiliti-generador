-- Amplia los catalogos genericos a Labenze y Requiez sin reescribir historial.
-- Se ejecuta despues de 2026_08_idelika_conceptos_catalogs.sql.
...
ALTER TABLE saas_catalog_sources
    ...
    CHECK (supplier IN ('cr-global','sonara','sunon','alma','lumbro','jome','lauco','idelika','conceptos','labenze','requiez'));
...
-- Las once fuentes comparten el mismo drive y elemento raiz. Conserva cualquier
-- configuracion previa y usa CR Global solo como semilla para las fuentes nuevas.
DO $$
DECLARE
...
BEGIN
    ...
    IF v_graph_drive_id IS NULL OR v_graph_root_item_id IS NULL THEN
        RAISE EXCEPTION 'CR Global catalog source is required to provision Labenze/Requiez';
    END IF;
    ...
    FROM (VALUES
        ('labenze', 'Labenze', 'labenze'),
        ('requiez', 'Requiez', 'requiez')
...
BEGIN
    ...
    LOOP
        ...
        v_definition := replace(
            v_original,
            $old$'cr-global','sonara','sunon','alma','lumbro','jome','lauco','idelika','conceptos'$old$,
            $new$'cr-global','sonara','sunon','alma','lumbro','jome','lauco','idelika','conceptos','labenze','requiez'$new$
        );
        v_definition := replace(
            v_definition,
            'CARDINALITY(p_enabled_suppliers) NOT BETWEEN 1 AND 9',
            'CARDINALITY(p_enabled_suppliers) NOT BETWEEN 1 AND 11'
        );
        v_definition := replace(
            v_definition,
            'jsonb_array_length(p_groups) NOT BETWEEN 0 AND 11',
            'jsonb_array_length(p_groups) NOT BETWEEN 0 AND 13'
        );
        IF v_definition IS NOT DISTINCT FROM v_original THEN
            RAISE EXCEPTION 'Labenze/Requiez migration could not update %', v_signature;
        END IF;
