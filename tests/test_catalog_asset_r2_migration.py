...
def test_checkpoint_binding_is_strict_and_resume_never_skips_current_gets(tmp_path):
    entry = _entry(PNG, "png", "image/png")
    source, _, _, _ = _write_fixture(tmp_path, [(entry, PNG)])
    prepared = SimpleNamespace(
    ...
    class Rpc:
        def call(self, name, payload):
            raise AssertionError("a finalized checkpoint must not replay RPCs")
...
def test_failed_execute_report_keeps_partial_attempts_and_never_certifies(tmp_path, monkeypatch):
    ...
    assert result == 2
    assert payload["certified"] is False
    assert payload["transfer"]["attempts"] == 1
